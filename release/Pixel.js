...
/*----------------------------------------*\
  ...
  @Last Modified time: 2024-03-22 18:04:26
\*----------------------------------------*/
...
class Pixel {
  constructor(id, midiDisplay) {
    this.midiDisplay = (...d) => this.enable && midiDisplay(...d);
    this._id = id;
    this._color = 0;
    this._lumTag = 0;
    this._blinkTag = 0;
  ...
  set enable(value) {
    this._active = !!value;
    if (this._active) {
      this.midiDisplay(RING_CHANNEL, this._id, this._value);
      this.midiDisplay(VALUE_CHANNEL, this._id, this._blinkValue);
      this.midiDisplay(VALUE_CHANNEL, this._id, this._strobValue);
      this.midiDisplay(TAG_CHANNEL, this._id, this._lumTag);
      this.midiDisplay(TAG_CHANNEL, this._id, this._blinkTag);
      this.midiDisplay(TAG_CHANNEL, this._id, this._strobTag);
      this.midiDisplay(COLOR_CHANNEL, this._id, this._color);
    }
  }
  get enable() {
    return this._active;
  ...
  set lumValue(value) {
    ...
    this.midiDisplay(VALUE_CHANNEL, this._id, this._lumValue);
  }
  get lumValue() {
  ...
  set black(value) {
    this.midiDisplay(COLOR_CHANNEL, this._id, !!value ? 0 : this._color);
  }
  set hue(value) {
  ...
  set demo(value) {
    this._demo = !!value;
    this.midiDisplay(TAG_CHANNEL, this._id, this._demo * 127);
  }
  get demo() {
    return this._demo;
  }
