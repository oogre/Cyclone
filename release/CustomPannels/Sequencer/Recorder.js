...
/*----------------------------------------*\
  ...
  @Last Modified time: 2024-03-25 19:30:44
\*----------------------------------------*/
...
class Recorder {
  ...
  set state(value) {
    if (!Object.values(Recorder.STATES).includes(value)) throw new Error(`Recorder : try set state to unknown (${value}) accepted : {${Object.keys(Recorder.STATES).map(key => `${key} : ${Recorder.STATES[key]}`).join(", ")}} `);
    if (this._state == value) return;
    const stateName = Object.keys(Recorder.STATES)[value];
    this._old_state = this._state;
    this._state = value;
    this[`on${(0, _tools.capitalize)(stateName)}`]();
  }
  get state() {
    return this._state;
  }
